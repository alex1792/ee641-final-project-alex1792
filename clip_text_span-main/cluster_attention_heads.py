...
def get_args_parser():
    parser = argparse.ArgumentParser("Cluster Attention Heads", add_help=False)
...
def main(args):
    ...
    print(f"Loading attention features from: {attn_path}")
    ...
    layers = list(range(num_layers - args.num_last_layers, num_layers))  # last N layers
    ...
    # 8. execute clustering - use command line arguments
    print(f"\n{'='*80}")
    print(f"Clustering Configuration")
    print(f"{'='*80}")
    print(f"Number of clusters: {args.n_clusters}")
    print(f"SVD rank: {args.w_ov_rank}")
    ...
    head_list, cluster_labels, proj_matrix = cluster_by_projected_features(
        ...
        text_features,
        text_lines,
        n_clusters=args.n_clusters,  # use command line arguments
        rank=args.w_ov_rank
    )
    ...
    #     # Check if current cluster should be skipped
    ...
    #         # Continue, but selection has been cleared
    #         pass
    ...
    output_file = os.path.join(
        args.output_dir, 
        f"head_clustering_projected_results_n{args.n_clusters}_{args.model}.txt"
    )
    ...
    # save ablation heads list - add strategy 4
    ...
    with open(ablation_file, "w") as f:
        ...
        # strategy 4: individually selected heads (interactive selection)
